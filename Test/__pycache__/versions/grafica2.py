...
# Calcula la media y desviación estándar de la tasa de acierto1 por plataforma y r/s
tasa_acierto1_por_plataforma_rs = data.groupby(['Plataforma', 'R/S'])['Tasa_acierto1']
media_tasa_acierto1_rs = tasa_acierto1_por_plataforma_rs.mean().unstack()
desviacion_tasa_acierto1_rs = tasa_acierto1_por_plataforma_rs.std().unstack()
...
# Calcula las medias y desviaciones de la tasa de acierto1 por atributo en las muestras sintéticas
media_acierto1_por_atributo = {}
desviacion_acierto1_por_atributo = {}
for atributo in atributos:
    ...
    data_grouped = filtered_data.groupby(interval_data)['Tasa_acierto1']
    media_acierto1_por_atributo[atributo] = data_grouped.mean()
    desviacion_acierto1_por_atributo[atributo] = data_grouped.std()
...
data_grouped_sigma = filtered_data_sigma.groupby(interval_data_sigma)['Tasa_acierto1']
media_acierto1_por_atributo['Sigma'] = data_grouped_sigma.mean()
desviacion_acierto1_por_atributo['Sigma'] = data_grouped_sigma.std()

# Gráfica para el atributo Color con intervalos en el eje x
plt.figure(figsize=(10, 6))
interval_data_color = pd.cut(data['Color'], bins=num_intervals)
media_acierto1_por_atributo['Color'] = data.groupby(interval_data_color)['Tasa_acierto1'].mean()
desviacion_acierto1_por_atributo['Color'] = data.groupby(interval_data_color)['Tasa_acierto1'].std()
media_acierto1_por_atributo['Color'].plot(style='.-', yerr=desviacion_acierto1_por_atributo['Color'], capsize=4)
plt.xlabel('Color')
plt.ylabel('Tasa de Acierto 1')
plt.title('Media y Desviación Típica de Tasa de Acierto 1 por Color')
...
for i, atributo in enumerate(['Gamma', 'Sigma', 'Brillo']):
    plt.figure(figsize=(10, 6))
    media_acierto1_por_atributo[atributo].plot(style=line_styles[i], yerr=desviacion_acierto1_por_atributo[atributo], capsize=4)
    plt.xlabel(atributo)
    plt.ylabel('Tasa de Acierto 1')
    plt.title(f'Media y Desviación Típica de Tasa de Acierto 1 por {atributo}')
    plt.xticks(rotation=45)
    plt.grid(True)
    plt.tight_layout()
